...
def closedPredicates = [friends, localRecovers, similar, usesAlcoholWord, usesSoberWord, attendsAA, retweets, replies,
	containsAlcoholWord, containsSoberWord, topics, 
	affect, social] as Set;
...
model.add( rule: (localRecovers(U) & attendsAA(U)) >> recovers(U), squared: true, weight : 1)
model.add( rule : (~(localRecovers(U)) & attendsAA(U)) >> ~(recovers(U)), squared:true, weight : 1)

log.info("Defining similarity collective rules..");
model.add( rule : (attendsAA(U1) & attendsAA(U2) & similar(U1, U2) & recovers(U2)) >> recovers(U1), squared:true, weight : 1)
model.add( rule : (attendsAA(U1) & attendsAA(U2) & similar(U1, U2) & recovers(U1)) >> recovers(U2), squared:true, weight : 1)
model.add( rule : (attendsAA(U1) & attendsAA(U2) & similar(U1, U2) & ~recovers(U2)) >> ~recovers(U1), squared:true, weight : 1)
model.add( rule : (attendsAA(U1) & attendsAA(U2) & similar(U1, U2) & ~recovers(U1)) >> ~recovers(U2), squared:true, weight : 1)

model.add( rule: (topics(U, '0') & attendsAA(U)) >> ~recovers(U), squared: true, weight : 1)
model.add( rule: (topics(U, '1') & attendsAA(U)) >> recovers(U), squared: true, weight : 1)
...
//	model.add( rule : (attendsAA(U1) & affect(U1) & friends(U1, U2) & usesAlcoholWord(U2, aw)) >> ~recovers(U1), squared:true,
//		weight : 0.5)
}
...
//	model.add( rule : (attendsAA(U1) & affect(U1) & friends(U1, U2) & usesSoberWord(U2, sw)) >> recovers(U1), squared:true,
//		weight : 0.5)
}
...
def dataDir = 'data'+java.io.File.separator+'recover_data'+ java.io.File.separator+ 'recover5'
...
//	def weightLearner = new PairedDualLearner(model, trainDB, labelsDB, config);
	weightLearner.learn();
	weightLearner.close();
