...
for(String aw : topAlcoholWords){
	...
	model.add( rule : (social(U1) & friends(U1, U2) & usesAlcoholWord(U2, aw)) >> ~recovers(U1), squared:true,
		weight : 1)
...
for(String sw : ['#recovery', 'sober', 'sobriety', 'recovery', '#sobriety']){
	...
	model.add( rule : (social(U1) & friends(U1, U2) & usesSoberWord(U2, sw)) >> recovers(U1), squared:true,
		weight : 1)
